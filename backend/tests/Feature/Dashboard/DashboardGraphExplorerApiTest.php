...
it('validates the bounded query contract and forbids internal or plugin fields', function (array $body): void {
    ...
    [['type' => 'overview', 'cursor' => str_repeat('c', DashboardGraphExplorerCursor::MAX_LENGTH + 1)]],
...
it('accepts the signed search cursors generated by the explorer at the HTTP boundary', function (): void {
    $user = task3ApiUser('Admin');
    $projectId = task3ApiProject();
    $repository = task3ApiRepositoryScope($projectId, str_repeat('a', 64));
    $fixture = task3ApiBindExplorer($this, $projectId, $repository['id'], $repository['graph_version']);
    $query = '/'.str_repeat('generale-soggetti-attivi-', 5);
    $cursor = (new DashboardGraphExplorerCursor)->encode(
        ...
        'search',
        $query,
        sprintf('%.17g|%s', 9000.0, $fixture['source_handle']),
    );

    expect(strlen($cursor))->toBeGreaterThan(512)
        ->toBeLessThanOrEqual(DashboardGraphExplorerCursor::MAX_LENGTH);
