...
it('preserves wiki markdown bytes including CRLF and a final newline', function () {
    $context = createWikiRevisionContext();
    $content = "# Routes\r\n\r\nGenerated by the plugin.\r\n";

    $response = $this->postJson(
        "/api/plugin/v1/runs/{$context['run_id']}/wiki/revisions",
        wikiPayload($context, ['content_markdown' => $content]),
        wikiHeaders($context),
    )->assertOk();

    $stored = DB::table('wiki_revisions')->where('id', $response->json('wiki_revision_id'))->value('content_markdown');

    expect($stored)->toBe($content)
        ->and(bin2hex((string) $stored))->toBe(bin2hex($content));
});
